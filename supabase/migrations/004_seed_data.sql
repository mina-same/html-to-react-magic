-- ============================================================
-- Fix assoc names + seed all data (users already exist)
-- Run in Supabase SQL Editor
-- ============================================================

-- fix assoc names
update public.profiles set assoc_name = 'جمعية تكاتف الخيرية'  where id = '00000000-0000-0000-0000-000000000002';
update public.profiles set assoc_name = 'جمعية رحمة للأيتام'   where id = '00000000-0000-0000-0000-000000000003';
update public.profiles set assoc_name = 'جمعية نور للتنمية'    where id = '00000000-0000-0000-0000-000000000004';
update public.profiles set assoc_name = 'جمعية الأمان الاجتماعي' where id = '00000000-0000-0000-0000-000000000005';
...
-- campaign requests (use influencer IDs by name to be safe)
insert into public.campaign_requests (assoc_id, influencer_id, type, budget, start_date, duration, message, status)
select '00000000-0000-0000-0000-000000000002', id, 'خيرية', 8000, '2026-06-01', 'أسبوعان',
  'نرغب في التعاون معك لحملة كسوة الشتاء، يشمل المحتوى قصصاً من الميدان وتحفيز المتبرعين.', 'pending'
from public.influencers where name = 'أبو خالد العوضي';

insert into public.campaign_requests (assoc_id, influencer_id, type, budget, start_date, duration, message, status)
select '00000000-0000-0000-0000-000000000002', id, 'توعوية', 12000, '2026-06-15', 'شهر',
  'حملة توعوية بأهمية دعم الطلاب، مقاطع قصيرة على تيك توك تستهدف الشباب.', 'pending'
from public.influencers where name = 'أحمد القحطاني';

insert into public.campaign_requests (assoc_id, influencer_id, type, budget, start_date, duration, message, status)
select '00000000-0000-0000-0000-000000000003', id, 'اجتماعية', 10000, '2026-06-10', 'أسبوعان',
  'نودّ التعاون في حملة توعوية لأهمية رعاية الأيتام على منصة X.', 'approved'
from public.influencers where name = 'هند الرشيدي';

insert into public.campaign_requests (assoc_id, influencer_id, type, budget, start_date, duration, message, status)
select '00000000-0000-0000-0000-000000000003', id, 'وثائقية', 6000, '2026-07-01', 'أسبوع',
  'تغطية وثائقية لمشروعنا الجديد لرعاية الأيتام في مدينة جدة.', 'pending'
from public.influencers where name = 'سالم الدوسري';

insert into public.campaign_requests (assoc_id, influencer_id, type, budget, start_date, duration, message, status)
select '00000000-0000-0000-0000-000000000004', id, 'تنموية', 5000, '2026-06-20', 'أسبوع',
  'حملة للتعريف ببرامجنا التدريبية للشباب واستقطاب المتطوعين.', 'matched'
from public.influencers where name = 'أبو خالد العوضي';

insert into public.campaign_requests (assoc_id, influencer_id, type, budget, start_date, duration, message, status)
select '00000000-0000-0000-0000-000000000005', id, 'طارئة', 9000, '2026-05-28', 'أسبوعان',
  'نحتاج دعمك لحملة عاجلة لجمع تبرعات للأسر المتضررة.', 'rejected'
from public.influencers where name = 'نورة العتيبي';

-- matches
insert into public.matches (assoc_id, influencer_id, status, start_date, budget, notes)
select '00000000-0000-0000-0000-000000000002', id, 'active', '2026-04-01', 14000,
  'حملة ناجحة — المؤثرة هند الرشيدي. 4 منشورات أسبوعياً على X.'
from public.influencers where name = 'هند الرشيدي';

insert into public.matches (assoc_id, influencer_id, status, start_date, budget, notes)
select '00000000-0000-0000-0000-000000000003', id, 'completed', '2026-02-15', 6000,
  'تغطية مميزة لمشاريع الرعاية، أنتجنا 2 فيديو وثائقي قصير.'
from public.influencers where name = 'سالم الدوسري';

insert into public.matches (assoc_id, influencer_id, status, start_date, budget, notes)
select '00000000-0000-0000-0000-000000000004', id, 'active', '2026-05-01', 5000,
  'تعاون لحملة فرص الشباب مع أبو خالد العوضي على إنستغرام.'
from public.influencers where name = 'أبو خالد العوضي';
