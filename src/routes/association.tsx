...
function Association() {
  ...
  const [campaignModal, setCampaignModal] = useState<{ open: boolean; inf?: Influencer }>({ open: false });
  ...
  function saveTask(data: Omit<Task, "id">) {
    if (taskModal.task) {
      setTasks(prev => prev.map(t => t.id === taskModal.task!.id ? { ...data, id: t.id } : t));
    } else {
  ...
  function saveInfluencer(data: Omit<Influencer, "id">) {
    if (infModal.inf) {
      setInfluencers(prev => prev.map(i => i.id === infModal.inf!.id ? { ...data, id: i.id } : i));
    } else {
  ...
  function renderPage() {
    switch (activePage) {
      case "overview":
        return <OverviewPage assocName={assocName} statContent={contentCount} onNavigate={setActivePage} />;
      case "profile":
        return <ProfilePage onAnalysisComplete={(name, count) => { if (name) setAssocName(name); setContentCount(c => c + count); }} />;
      case "team":
        return <TeamPage employees={employees} onAdd={() => setEmpModal(true)} onDelete={deleteEmployee} />;
      case "tasks":
        return (
          <TasksPage
            tasks={tasks}
  ...
  return (
    <div dir="rtl" style={{ display: "flex", height: "100dvh", background: "#f4f7f5", fontFamily: "'Tajawal','Cairo',sans-serif", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
        {/* Topbar */}
        <div style={{ height: 58, background: "white", borderBottom: "1px solid rgba(45,122,82,.12)", display: "flex", alignItems: "center", padding: "0 22px", gap: 12, flexShrink: 0 }}>
          {/* Mobile menu button */}
          <button
            className="md:hidden"
            onClick={() => setSidebarOpen(true)}
            style={{ width: 34, height: 34, borderRadius: 8, border: "1px solid rgba(45,122,82,.15)", background: "white", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: "1rem", flexShrink: 0 }}>
            ☰
          </button>
          ...
          {assocName && (
            <span style={{ fontSize: ".83rem", fontWeight: 600, color: "#2d7a52", background: "#e8f5ee", padding: "5px 12px", borderRadius: 20, whiteSpace: "nowrap" }}>
              {assocName}
            </span>
          )}

          <div style={{ width: 32, height: 32, borderRadius: 8, border: "1.5px solid rgba(45,122,82,.12)", background: "white", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: ".95rem", position: "relative", flexShrink: 0 }}>
            🔔
            {urgentTasks > 0 && (
              <span style={{ position: "absolute", top: 5, left: 5, width: 6, height: 6, background: "#c9a84c", borderRadius: "50%", border: "2px solid white" }} />
            )}
          </div>
        </div>
