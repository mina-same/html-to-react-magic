...
<aside class="sidebar">
  ...
  <div class="sb-admin-badge">
    ...
    <span class="sb-admin-txt">لوحة إدارة المنصة</span>
  </div>
  <nav class="sb-nav">
    ...
    <div class="sb-sec">إدارة الأطراف</div>
    <button class="sbi" onclick="showPage('orgs',this)"><span class="sbi-ic">🏛</span> الجمعيات <span class="sbi-badge red" id="sb-new-orgs">3</span></button>
    <button class="sbi" onclick="showPage('influencers',this)"><span class="sbi-ic">⭐</span> المؤثرون</button>
    <div class="sb-sec">الحملات والربط</div>
    <button class="sbi" onclick="showPage('requests',this)"><span class="sbi-ic">📣</span> طلبات الحملات <span class="sbi-badge gold" id="sb-pending-req">5</span></button>
    <button class="sbi" onclick="showPage('matches',this)"><span class="sbi-ic">🔗</span> الربط والتعاقد</button>
    <div class="sb-sec">التقارير</div>
    <button class="sbi" onclick="showPage('reports',this)"><span class="sbi-ic">📊</span> التقارير المالية</button>
    <button class="sbi" onclick="showPage('settings',this)"><span class="sbi-ic">⚙</span> إعدادات المنصة</button>
  </nav>
  <div class="sb-bot">
    <button class="sbi" style="color:rgba(255,100,100,.45);font-size:.82rem" onclick="window.location.href='dashboard.html'"><span class="sbi-ic">↩</span> الخروج</button>
  </div>
</aside>
...
<div class="main">
  <header class="topbar">
    ...
    <div class="tb-tag">🔐 مدير ساعِد</div>
    <button onclick="window.location.href='association-dashboard.html'" style="padding:6px 13px;border-radius:7px;background:var(--g5);border:1px solid rgba(45,122,82,.15);font-family:Tajawal,sans-serif;font-size:.76rem;font-weight:600;color:var(--g2);cursor:pointer">عرض داشبورد الجمعيات ↗</button>
  </header>
  ...
  <div class="content">
    ...
    <div class="page active" id="page-overview">
      <div class="kpi-row">
        <div class="kpi-card g">
          <div class="kpi-num" style="color:var(--g2)" id="ov-orgs">12</div>
          <div class="kpi-lbl">جمعية مسجلة</div>
          <div class="kpi-sub" style="color:var(--g3)" id="ov-orgs-sub">3 جديدة تنتظر الموافقة</div>
        </div>
        <div class="kpi-card gold">
          <div class="kpi-num" style="color:#92400e" id="ov-infs">6</div>
          <div class="kpi-lbl">مؤثر في المنصة</div>
          <div class="kpi-sub" style="color:#d97706">4 نشطون حالياً</div>
        </div>
        <div class="kpi-card blue">
          <div class="kpi-num" style="color:#1e40af" id="ov-reqs">5</div>
          <div class="kpi-lbl">طلب حملة معلق</div>
          <div class="kpi-sub" style="color:#2563eb">⚡ تحتاج مراجعة</div>
        </div>
        <div class="kpi-card red">
          <div class="kpi-num" style="color:#b91c1c" id="ov-revenue">24,500</div>
          <div class="kpi-lbl">إيرادات هذا الشهر (ر.س)</div>
          <div class="kpi-sub" style="color:#dc2626">↑ 18% عن الشهر الماضي</div>
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
        ...
        <div class="sec-card">
          <div class="sec-head">
            ...
            <div><div class="sec-title">آخر الجمعيات المسجلة</div><div class="sec-sub">تنتظر المراجعة</div></div>
            <button class="btn-ghost" style="margin-right:auto;font-size:.72rem" onclick="showPage('orgs',document.querySelector('[onclick*=orgs]'))">عرض الكل</button>
          </div>
        ...
        <div class="sec-card">
          <div class="sec-head">
            <div class="sec-ic">📣</div>
            <div><div class="sec-title">آخر طلبات الحملات</div><div class="sec-sub">تحتاج ربط بمؤثر</div></div>
            <button class="btn-ghost" style="margin-right:auto;font-size:.72rem" onclick="showPage('requests',document.querySelector('[onclick*=requests]'))">عرض الكل</button>
          </div>
          <div class="sec-body" style="padding:0">
            <table class="tbl">
    ...
    <div class="page" id="page-orgs">
      <div class="toolbar">
        <div class="search-bar"><span>🔍</span><input id="orgSearch" placeholder="بحث بالاسم أو المنطقة..." oninput="renderOrgs()"></div>
        <select id="orgStatusFilter" onchange="renderOrgs()" style="padding:7px 12px;border-radius:7px;border:1.5px solid var(--border);font-family:Tajawal,sans-serif;font-size:.82rem;color:var(--tm);background:white">
        ...
        <button class="btn-primary" onclick="openOrgModal(null)" style="margin-right:auto">+ إضافة جمعية</button>
      </div>
      <div class="sec-card">
        <div class="sec-body" style="padding:0">
          <table class="tbl">
    ...
    <div class="page" id="page-influencers">
      <div class="toolbar">
        <div class="search-bar"><span>🔍</span><input id="infSearch" placeholder="بحث بالاسم أو المنصة..." oninput="renderAdminInfs()"></div>
        <select id="infPlatFilter" onchange="renderAdminInfs()" style="padding:7px 12px;border-radius:7px;border:1.5px solid var(--border);font-family:Tajawal,sans-serif;font-size:.82rem;color:var(--tm);background:white">
        ...
        <button class="btn-primary" onclick="openInfModal(null)" style="margin-right:auto">+ إضافة مؤثر</button>
      </div>
      <div class="inf-grid" id="adminInfGrid"></div>
    </div>

    <!-- ══════ PAGE: REQUESTS ══════ -->
    <div class="page" id="page-requests">
      <div class="toolbar">
        <select id="reqStatusFilter" onchange="renderRequests()" style="padding:7px 12px;border-radius:7px;border:1.5px solid var(--border);font-family:Tajawal,sans-serif;font-size:.82rem;color:var(--tm);background:white">
    ...
    <div class="page" id="page-matches">
      <div class="toolbar">
        <button class="btn-primary" onclick="openMatchModal()">+ ربط جمعية بمؤثر</button>
        <select id="matchStatusFilter" onchange="renderMatches()" style="padding:7px 12px;border-radius:7px;border:1.5px solid var(--border);font-family:Tajawal,sans-serif;font-size:.82rem;color:var(--tm);background:white;margin-right:auto">
    ...
    <div class="page" id="page-reports">
      <div class="kpi-row">
        <div class="kpi-card g">
          <div class="kpi-num" style="color:var(--g2)">24,500</div>
          <div class="kpi-lbl">إيرادات هذا الشهر (ر.س)</div>
          <div class="kpi-sub" style="color:var(--g3)">↑ 18%</div>
        </div>
        <div class="kpi-card gold">
          <div class="kpi-num" style="color:#92400e">8</div>
          <div class="kpi-lbl">حملة مكتملة</div>
          <div class="kpi-sub" style="color:#d97706">هذا الشهر</div>
        </div>
        <div class="kpi-card blue">
          <div class="kpi-num" style="color:#1e40af">3.1M</div>
          <div class="kpi-lbl">إجمالي الوصول</div>
          <div class="kpi-sub" style="color:#2563eb">عبر كل الحملات</div>
        </div>
        <div class="kpi-card red">
          <div class="kpi-num" style="color:#b91c1c">92%</div>
          <div class="kpi-lbl">رضا الجمعيات</div>
          <div class="kpi-sub" style="color:#dc2626">من آخر 20 حملة</div>
        </div>
      </div>
      <div class="sec-card">
        <div class="sec-head"><div class="sec-ic">📊</div><div><div class="sec-title">ملخص الإيرادات</div></div></div>
        <div class="sec-body">
          <table class="tbl">
    ...
    <div class="page" id="page-settings">
      <div class="sec-card">
        <div class="sec-head"><div class="sec-ic">⚙</div><div><div class="sec-title">إعدادات المنصة</div></div></div>
        <div class="sec-body">
          <div style="display:grid;gap:13px;max-width:480px">
            <div class="m-field"><label>نسبة عمولة ساعِد %</label><input class="m-input" type="number" value="15" min="0" max="100"></div>
            <div class="m-field"><label>الحد الأدنى لميزانية الحملة (ر.س)</label><input class="m-input" type="number" value="250"></div>
            <div class="m-field"><label>البريد الإلكتروني للإشعارات</label><input class="m-input" type="email" value="admin@saaid.com" dir="ltr"></div>
            <button class="btn-primary" onclick="alert('✓ تم حفظ الإعدادات')">حفظ الإعدادات</button>
          </div>
        </div>
      </div>
    </div>
...
<!-- ORG MODAL -->
<div class="overlay hidden" id="orgOverlay" onclick="if(event.target===this)closeOrgModal()">
  <div class="modal">
    <div class="m-head">
      <span class="m-title" id="orgModalTitle">إضافة جمعية</span>
      <button class="m-close" onclick="closeOrgModal()">✕</button>
    </div>
    <div class="m-body">
      <div class="m-row">
        <div class="m-field"><label>اسم الجمعية</label><input class="m-input" id="org-name" placeholder="جمعية تكاتف الخيرية"></div>
        <div class="m-field"><label>رقم الترخيص</label><input class="m-input" id="org-license" placeholder="xxxxxxxx" dir="ltr"></div>
      </div>
      <div class="m-row">
        <div class="m-field"><label>المنطقة</label>
          <select class="m-select" id="org-region">
        ...
        <div class="m-field"><label>الحالة</label>
          <select class="m-select" id="org-status">
      ...
      <div class="m-row">
        <div class="m-field"><label>البريد الإلكتروني</label><input class="m-input" id="org-email" type="email" placeholder="info@assoc.org" dir="ltr"></div>
        <div class="m-field"><label>رقم الجوال</label><input class="m-input" id="org-phone" type="tel" placeholder="05xxxxxxxx" dir="ltr"></div>
      </div>
      <div class="m-field"><label>ملاحظات</label><textarea class="m-textarea" id="org-notes" placeholder="أي ملاحظات إضافية..."></textarea></div>
    </div>
    <div class="m-footer">
      <button class="btn-danger" id="orgDeleteBtn" onclick="deleteOrg()" style="display:none;margin-left:auto">حذف</button>
      <button class="btn-ghost" onclick="closeOrgModal()">إلغاء</button>
      <button class="btn-primary" onclick="saveOrg()">حفظ</button>
    </div>
  </div>
</div>

<!-- INF MODAL -->
<div class="overlay hidden" id="infOverlay" onclick="if(event.target===this)closeInfModal()">
  <div class="modal">
    <div class="m-head">
      <span class="m-title" id="infModalTitle">إضافة مؤثر</span>
      <button class="m-close" onclick="closeInfModal()">✕</button>
    </div>
    <div class="m-body">
      <div class="m-row">
        <div class="m-field"><label>الاسم</label><input class="m-input" id="inf-name" placeholder="اسم المؤثر"></div>
        <div class="m-field"><label>المنصة الرئيسية</label>
          <select class="m-select" id="inf-platform">
            <option>Instagram</option><option>X</option><option>TikTok</option><option>YouTube</option><option>Snapchat</option>
          </select>
        </div>
      </div>
      <div class="m-row">
        <div class="m-field"><label>عدد المتابعين</label><input class="m-input" id="inf-followers" type="number" placeholder="مثال: 150000" dir="ltr"></div>
        <div class="m-field"><label>نسبة التفاعل %</label><input class="m-input" id="inf-eng" type="number" step="0.1" placeholder="مثال: 4.5" dir="ltr"></div>
      </div>
      <div class="m-row">
        <div class="m-field"><label>السعر المبدئي (ر.س)</label><input class="m-input" id="inf-price" type="number" placeholder="يبدأ من 250" min="250" dir="ltr"></div>
        <div class="m-field"><label>حالة العقد</label>
          <select class="m-select" id="inf-status">
            <option value="active">نشط</option><option value="pending">قيد التفاوض</option><option value="ended">انتهى العقد</option>
          </select>
        </div>
      </div>
      <div class="m-field"><label>تخصص المحتوى</label><input class="m-input" id="inf-niche" placeholder="مثال: محتوى خيري، ديني، عائلي"></div>
      <div class="m-field"><label>ملاحظات العقد</label><textarea class="m-textarea" id="inf-notes" placeholder="شروط، تواريخ، تفاصيل التعاقد..."></textarea></div>
    </div>
    <div class="m-footer">
      <button class="btn-danger" id="infDeleteBtn" onclick="deleteInf()" style="display:none;margin-left:auto">حذف</button>
      <button class="btn-ghost" onclick="closeInfModal()">إلغاء</button>
      <button class="btn-primary" onclick="saveInf()">حفظ</button>
    </div>
  </div>
</div>

<!-- REQUEST REVIEW MODAL -->
<div class="overlay hidden" id="reqOverlay" onclick="if(event.target===this)closeReqModal()">
  <div class="modal">
    <div class="m-head">
      <span class="m-title">مراجعة طلب الحملة</span>
      <button class="m-close" onclick="closeReqModal()">✕</button>
    </div>
    <div class="m-body" id="reqModalBody"></div>
    <div class="m-footer">
      <button class="btn-danger" onclick="updateReqStatus('rejected');closeReqModal()">رفض الطلب</button>
      <button class="btn-ghost" onclick="closeReqModal()">إغلاق</button>
      <button class="btn-gold" onclick="openMatchFromReq()">ربط بمؤثر ←</button>
      <button class="btn-primary" onclick="updateReqStatus('completed');closeReqModal()">تم التنفيذ ✓</button>
    </div>
  </div>
</div>

<!-- MATCH MODAL -->
<div class="overlay hidden" id="matchOverlay" onclick="if(event.target===this)closeMatchModal()">
  <div class="modal" style="width:480px">
    <div class="m-head">
      <span class="m-title">ربط جمعية بمؤثر</span>
      <button class="m-close" onclick="closeMatchModal()">✕</button>
    </div>
    <div class="m-body">
      <div class="m-field"><label>الجمعية</label>
        <select class="m-select" id="match-org"></select>
      </div>
      <div class="m-field"><label>المؤثر</label>
        <select class="m-select" id="match-inf"></select>
      </div>
      <div class="m-row">
        <div class="m-field"><label>قيمة العقد (ر.س)</label><input class="m-input" id="match-value" type="number" min="250" placeholder="500" dir="ltr"></div>
        <div class="m-field"><label>الحالة</label>
          <select class="m-select" id="match-status">
            <option value="pending">قيد التفاوض</option><option value="active">نشط</option><option value="completed">مكتمل</option>
          </select>
        </div>
      </div>
      <div class="m-field"><label>ملاحظات</label><textarea class="m-textarea" id="match-notes" placeholder="تفاصيل الحملة، شروط التعاقد..."></textarea></div>
    </div>
    <div class="m-footer">
      <button class="btn-ghost" onclick="closeMatchModal()">إلغاء</button>
      <button class="btn-primary" onclick="saveMatch()">حفظ الربط</button>
    </div>
  </div>
</div>
