...
export default function VideoEditorModal({
  ...
  initialTransitionStyle = "slide",
  initialShowOutro = true,
  onSave,
  onClose,
}: Props) {
  ...
  useEffect(() => {
    ...
    if (!player) return;
    const h = (e: { detail: { frame?: number } }) => {
      const f = e.detail?.frame ?? 0;
      setCurrentFrame(f);
      let idx = slides.length - 1;
      for (let i = 0; i < slides.length; i++) {
        if (f < slideStarts[i] + slides[i].durationFrames) {
          idx = i;
          break;
        }
      }
  ...
  const updateText = (i: number, v: string) =>
    setSlides((p) => p.map((s, j) => (j === i ? { ...s, line: v } : s)));
  const updateIcon = (i: number, v: string) =>
    setSlides((p) => p.map((s, j) => (j === i ? { ...s, icon: v } : s)));
  const updateDuration = (i: number, v: number) =>
    setSlides((p) => p.map((s, j) => (j === i ? { ...s, durationFrames: v } : s)));
  ...
  const onDrop = (e: React.DragEvent, i: number) => {
    ...
    const next = [...slides];
    const [rm] = next.splice(dragIdx, 1);
    next.splice(i, 0, rm);
    setSlides(next);
    setSelectedIdx(i);
    setDragIdx(null);
  ...
  const handleSave = () => {
    onSave(assembledText, slideFrames, showLogo);
    onClose();
  };
          ...
          >
            <Player
              ...
              inputProps={{
                ...
                brandColors: aiBrand
                  ? [
                      parseBrandColors(aiBrand).primary,
                      parseBrandColors(aiBrand).secondary,
                      parseBrandColors(aiBrand).accent,
                    ]
                  : undefined,
                transitionStyle,
                slideFrames,
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div
                        style={{
                          fontSize: "11px",
                          color: isSel ? RS.text : RS.textSub,
            ...
            <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden" }}>
              ...
              <PropRow label="duration">
                <div style={{ marginTop: 4 }}>
                  ...
                  >
                    <span
                      style={{ fontSize: "10px", color: RS.textMuted, fontFamily: "monospace" }}
                    >
          ...
          {tab === "settings" && (
            <div style={{ flex: 1, overflowY: "auto" }}>
              ...
              <LayerRow
                ...
                color={RS.yellow}
                visible={showOutro}
                onToggle={() => setShowOutro((v) => !v)}
              />

              <SectionHeader label="slide transitions" />
              <div
                style={{ padding: "8px 12px", display: "flex", flexDirection: "column", gap: 5 }}
              >
              ...
              <div style={{ padding: "10px 12px" }}>
                ...
                <a
                  ...
                  onMouseEnter={(e) =>
                    ((e.currentTarget as HTMLAnchorElement).style.borderColor = RS.accent)
                  }
                  onMouseLeave={(e) =>
                    ((e.currentTarget as HTMLAnchorElement).style.borderColor = RS.border)
                  }
                >
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", height: 28, flexShrink: 0 }}>
              ...
              <div
                style={{ flex: 1, position: "relative", borderBottom: `1px solid ${RS.borderSub}` }}
              >
                <div
                  style={{
                    position: "absolute",
                    ...
                    top: 4,
                    height: 20,
                    width: totalFrames * PX_PER_FRAME,
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        width: w,
                        height: 28,
                        background: sel ? RS.accentDim : act ? "rgba(56,139,253,.08)" : RS.surface,
                    ...
                    >
                      ...
                      {w > 38 && (
                        <span
                          style={{
                            fontSize: "9px",
                            color: sel ? RS.text : RS.textMuted,
                            fontFamily: "monospace",
                            overflow: "hidden",
                            textOverflow: "ellipsis",
                            whiteSpace: "nowrap",
              ...
              >
                {audioUrl ? (
                  ...
                  >
                    ...
                    >
                      {Array.from({ length: Math.floor((totalFrames * PX_PER_FRAME) / 3.5) }).map(
                        (_, j) => (
                          <div
                            key={j}
                            style={{
                              width: 1.5,
                              flexShrink: 0,
                              height: `${28 + Math.sin(j * 0.9) * 15 + Math.sin(j * 1.7) * 10}%`,
                  ...
                  <div
                    style={{
                      ...
                      height: 20,
                      width: totalFrames * PX_PER_FRAME,
                      border: `1px dashed ${RS.borderSub}`,
                      borderRadius: 3,
                      display: "flex",
                      alignItems: "center",
                      paddingLeft: 6,
