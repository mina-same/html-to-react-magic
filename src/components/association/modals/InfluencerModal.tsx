...
export default function InfluencerModal({ influencer, onSave, onDelete, onClose }: Props) {
  ...
  const [platform,   setPlatform]   = useState<Influencer["platform"]>(influencer?.platform ?? "Instagram");
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent style={{ maxWidth: 480, fontFamily: "'Tajawal','Cairo',sans-serif", direction: "rtl" }}>
        <DialogHeader>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: ".72rem", padding: "3px 10px", borderRadius: 20, fontWeight: 700, background: "#fef9c3", color: "#92400e" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              ...
              <Input value={name} onChange={e => setName(e.target.value)} placeholder="اسم المؤثر" style={sel} />
            </div>
            <div>
              <label style={lbl}>المنصة الرئيسية</label>
              <select value={platform} onChange={e => setPlatform(e.target.value as Influencer["platform"])} style={sel}>
                {(["Instagram","X","TikTok","YouTube","Snapchat"] as const).map(p => <option key={p} value={p}>{p}</option>)}
              </select>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              ...
              <Input type="number" dir="ltr" value={followers || ""} onChange={e => setFollowers(Number(e.target.value))} placeholder="150000" style={sel} />
            </div>
            <div>
              <label style={lbl}>نسبة التفاعل %</label>
              <Input type="number" dir="ltr" step="0.1" value={engagement || ""} onChange={e => setEngagement(Number(e.target.value))} placeholder="4.5" style={sel} />
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
              ...
              <select value={status} onChange={e => setStatus(e.target.value as Influencer["status"])} style={sel}>
            ...
            <div>
              ...
              <Input type="number" dir="ltr" value={campaigns || ""} onChange={e => setCampaigns(Number(e.target.value))} placeholder="0" style={sel} />
            </div>
          </div>

          <div>
            <label style={lbl}>تخصص المحتوى</label>
            <Input value={niche} onChange={e => setNiche(e.target.value)} placeholder="مثال: محتوى خيري، ديني، عائلي..." style={sel} />
          </div>
        ...
        <DialogFooter style={{ justifyContent: "space-between", display: "flex", gap: 8, flexDirection: "row" }}>
          {!isNew && (
            <Button variant="destructive" size="sm" onClick={() => { onDelete(influencer!.id!); onClose(); }}>حذف</Button>
          )}
